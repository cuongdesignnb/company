...
?>
<article class="cb-certificate-card<?php echo $expired ? ' is-expired' : ''; ?><?php echo $demo ? ' is-demo' : ''; ?>">
    <a class="cb-certificate-document" href="<?php echo esc_url(get_permalink($certificate_id)); ?>">
        ...
        <?php endif; ?>
        <?php if ($demo) : ?><span class="cb-certificate-status is-demo"><?php echo esc_html(cb_theme_lang() === 'zh' ? '演示文件' : 'Demo document'); ?></span><?php elseif ($expired && !$compact) : ?><span class="cb-certificate-status is-expired"><?php echo esc_html(cb_theme_lang() === 'zh' ? '已失效' : 'Expired'); ?></span><?php endif; ?>
    </a>
